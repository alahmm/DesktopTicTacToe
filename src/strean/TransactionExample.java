...
class TransactionExample {
    ...
    public static long calcSumOfCanceledTransOnNonEmptyAccounts(List<Account> accounts) {
        ...
        return accounts.stream().filter(f -> f.getBalance() > 0).flatMap(s -> s.getTransactions().stream()
                .filter(c -> c.getState().equals(State.CANCELED)))
                .map(Transaction::getSum).mapToLong(i ->i).sum();
